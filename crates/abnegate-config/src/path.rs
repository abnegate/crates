...
/// home directory.
pub fn config_dir(application: &str) -> Result<PathBuf, ConfigError> {
...
pub fn config_path(application: &str) -> Result<PathBuf, ConfigError> {
...
fn configured(application: &str, suffix: &str) -> Option<PathBuf> {
    env::var_os(variable(application, suffix))
        .filter(|value| !value.is_empty())
        .map(PathBuf::from)
}

fn variable(application: &str, suffix: &str) -> String {
...
mod tests {
    ...
    fn overrides_are_named_for_the_application() {
        assert_eq!(variable("example", PATH_VARIABLE), "EXAMPLE_CONFIG_PATH");
        assert_eq!(
            variable("example", DIRECTORY_VARIABLE),
            "EXAMPLE_CONFIG_DIR"
        );
    }
    ...
    fn override_names_are_valid_variable_names() {
        assert_eq!(variable("my-app", PATH_VARIABLE), "MY_APP_CONFIG_PATH");
        assert_eq!(variable("my.app", PATH_VARIABLE), "MY_APP_CONFIG_PATH");
        assert_eq!(variable("my app", PATH_VARIABLE), "MY_APP_CONFIG_PATH");
    }
    ...
    fn an_unset_override_falls_through() {
        assert!(configured("abnegate-config-no-such-application", PATH_VARIABLE).is_none());
    }
}
