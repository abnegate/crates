...
pub const MAXIMUM_TOOL_OUTPUT_CHARACTERS: usize = 8_000;
...
///
/// Sits above [`MAXIMUM_TOOL_OUTPUT_CHARACTERS`] and the framing around it, so a tool
...
pub const MAXIMUM_TOOL_MESSAGE_CHARACTERS: usize =
    MAXIMUM_TOOL_OUTPUT_CHARACTERS + TOOL_FRAMING_CHARACTERS;
...
pub const MAXIMUM_PREVIEW_CHARACTERS: usize = 400;
...
/// Cap `text` at `maximum_characters`, dropping the middle rather than the tail.
///
...
/// no-op.
pub(crate) fn trim_middle(text: &str, maximum_characters: usize) -> String {
...
mod tests {
    ...
    fn trim_middle_keeps_head_and_tail_inside_the_budget() {
        ...
        let trimmed = trim_middle(&text, MAXIMUM_TOOL_MESSAGE_CHARACTERS);
        ...
        assert!(trimmed.chars().count() <= MAXIMUM_TOOL_MESSAGE_CHARACTERS);
        assert_eq!(
            trim_middle("hello", MAXIMUM_TOOL_MESSAGE_CHARACTERS),
            "hello"
        );
        assert_eq!(
            trim_middle(&trimmed, MAXIMUM_TOOL_MESSAGE_CHARACTERS),
            trimmed,
