...
pub(crate) fn serialize<S: Serializer>(
    ...
    serializer.serialize_u64(whole(*duration))
}
...
mod tests {
    ...
    #[test]
    fn a_duration_is_written_as_whole_milliseconds() {
        let carrier = Carrier {
        ...
        assert_eq!(
            ...
            r#"{"duration":1500,"timeout":300000}"#
        );
    }
    ...
    fn an_absent_timeout_is_written_as_null_and_read_back_from_null_or_nothing() {
        ...
        assert_eq!(
            serde_json::to_string(&carrier).unwrap(),
            r#"{"duration":0,"timeout":null}"#
        );
        for line in [r#"{"duration":0,"timeout":null}"#, r#"{"duration":0}"#] {
    ...
    fn the_largest_value_round_trips_and_a_longer_duration_saturates() {
        let largest = format!(r#"{{"duration":{},"timeout":{}}}"#, u64::MAX, u64::MAX);
        let carrier: Carrier = serde_json::from_str(&largest).unwrap();

        assert_eq!(carrier.duration, Duration::from_millis(u64::MAX));
    ...
    fn a_negative_or_fractional_value_is_refused() {
        for line in [
            r#"{"duration":-1}"#,
            r#"{"duration":1.5}"#,
            r#"{"duration":0,"timeout":-1}"#,
            r#"{"duration":0,"timeout":1.5}"#,
        ] {
